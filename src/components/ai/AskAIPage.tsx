...
async function callWorkforceAI(question: string, docContext?: string): Promise<string> {
...
function buildPlainText(entry: OutputEntry): string {
  ...
  lines.push('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  lines.push('This report was generated by Progression Workforce Intelligence.');
  lines.push('Data is based on your internal workforce records.');
  return lines.join('\n');
}
...
function EmailModal({ entry, onClose }: { entry: OutputEntry; onClose: () => void }) {
  ...
  function send() {
    ...
    const subject = encodeURIComponent(`Progression AI: ${entry.question}`);
    ...
    <>
      <div className="h-full overflow-y-auto">
        <div className="max-w-4xl mx-auto px-12 py-10">
          ...
          {/* Missing context nudge */}
          {entry.needsMoreContext && (
...
export function AskAIPage({ initialQuestion, onNavigate }: Props) {
  ...
  async function sendMessage(text: string) {
    ...
    const finalize = (text: string, results: QueryResult[], needsMoreContext = false) => {
      const aiMsg: ChatMessage = { id: makeId(), role: 'assistant', text, results, timestamp: new Date() };
      const output: OutputEntry = { id: outputId, question: trimmed, answer: text, results, timestamp: new Date(), needsMoreContext };
      setConversations(prev => prev.map(c => {
        if (c.id !== activeId) return c;
    ...
    if (response.needsAI) {
      try {
        const aiText = await callWorkforceAI(trimmed, docContext);
        const needsMoreContext = !docContext && detectsMissingContext(aiText);
        finalize(aiText, [], needsMoreContext);
      } catch {
        ...
        finalize(fallback, [], isBudgetQ);
      }
      return;
    }

    setTimeout(() => finalize(response.text, response.results ?? []), 400 + Math.random() * 300);
  }
