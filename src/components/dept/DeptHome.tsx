...
export function DeptHome({ onSelectDept, onAskAI, onNavigateToPipeline, onNavigate }: Props) {
  ...
  function buildExportContent() {
    ...
    return [
      ...
      `Attrition Risk Score:      ${summary.attritionScore.score}/100 — ${summary.attritionScore.riskLabel}`,
      `No Check-In (overdue):     ${summary.overdueCheckIns + summary.criticalCheckIns}`,
      '',
      'PRIORITY ALERTS',
      '─────────────────────────────────────',
      ...attentionItems.map(i => `  [${i.urgency.toUpperCase()}] ${i.headline} — ${i.why}`),
      '',
      'HIGHLIGHTS',
      '─────────────────────────────────────',
