...
export const TOUR_STEPS: TourStep[] = [
...

  {
    ...
    body: 'Three headline numbers for the selected department: the percentage of staff below their expected competency level, the average size of those gaps, and the total number of skills being tracked. These update live as you apply filters.',
  ...
  },
  {
    ...
    body: 'Skills where 85%+ of the team are below the expected level are flagged as critical here. These represent the highest business risk and are the starting point for training prioritisation.',
    anchorId: 'heatmap-alert-bar',
    anchorOffsetX: 0.5, anchorOffsetY: 0.5,
    side: 'bottom',
  },
  {
    ...
    view: 'heatmap',
    title: 'Heatmap Grid',
    body: 'Each row is a skill; each column is a location or team. The colour shows how far that group is from the expected level — sky blue means exceeding target, green is on track, and the spectrum through amber, orange, and red reflects increasing gap severity. Click any cell to open the drill-down sidebar.',
  ...
  },
  {
    id: 'heatmap-4',
    view: 'heatmap',
    title: 'Gap Severity Legend',
    body: 'The colour scale runs from sky blue (exceeding target) through green (on track) to deep red (critical, 85%+ below target). A pattern of dark cells across a row means a team-wide weakness; a pattern down a column points to a location-specific issue.',
  ...
  },
  {
    ...
    body: 'Clicking a cell opens this sidebar for the selected skill. The three cards at the top show the percentage below target, the average competency level vs the expected level, and the worst-performing location or team.',
    anchorId: 'drilldown-stats',
    anchorOffsetX: 0.5, anchorOffsetY: 0.5,
    side: 'left',
  },
  {
    id: 'heatmap-6',
    view: 'heatmap',
    title: 'Promotion Pipeline Bridge',
    body: 'This section connects skill gaps directly to the promotion pipeline. It shows which promotion candidates already meet this skill\'s next-level criteria (highlighted in teal) and which are being blocked by it (highlighted in red) — giving you an instant read on who to develop first.',
    anchorId: 'drilldown-pipeline',
    anchorOffsetX: 0.5, anchorOffsetY: 0.4,
    side: 'left',
  },
  {
    ...
    body: 'The bars show the gap severity broken down by location (or team, depending on the filter). Each bar includes the headcount, the percentage below target, and the actual vs expected level. This makes it easy to spot whether a gap is spread evenly or concentrated in one office.',
    anchorId: 'drilldown-breakdown',
    anchorOffsetX: 0.5, anchorOffsetY: 0.3,
    side: 'left',
  },
  {
    ...
    body: 'Context-aware action shortcuts at the bottom of the panel. If promotion-ready candidates exist for this skill, you get a direct link to view them in the pipeline. Otherwise you see options to find mentors, set this as a team focus skill, or export the gap report.',
    anchorId: 'drilldown-actions',
    anchorOffsetX: 0.5, anchorOffsetY: 0.5,
    side: 'left',
  },
  ...
  },
  {
    ...
    body: 'Pre-built questions for the most common strategic workforce scenarios — retention risk, skills strategy, succession planning, restructuring analysis. Each generates a full data-backed briefing and is a good starting point before a meeting.',
  ...
  },
  {
    ...
    body: 'Responses appear here as structured briefings — not chat bubbles. Each output includes a summary, supporting data in named result blocks, and contextual navigation links to jump directly to the relevant heatmap, pipeline, or benchmark view.',
