...
- **Primary (indices 0–3):** Org Health, Below Expected Level, Promotable Now, Stalled 24M+
- **Secondary (indices 4–7):** Managers, Industry Rank, Attrition Risk, No Check-In
...
> **Scale warning:** All functions in this section are synchronous and calibrated against the current mock dataset (~85 people, 11 managers, 8 peer companies). They run on the main thread inside a `useMemo`. Do not connect to real data at scale without first moving compute off the main thread (e.g. a Web Worker or server-side API call) — the aggregation loops in `computeExecSummary()` are O(people × criteria) and will block the UI at several hundred employees.
